...
_HEIGHT( 1500 ),
SphereCollider( _pos, _PLAYER_COLLIDER_RADIUS, OBJECT_TAG_PLAYER ),
_pos( ),
_dir( 0, 0, 1 ) {
	CameraPtr camera = Camera::getTask( );
	camera->setCamera( _pos, _dir );
...
void Player::updatePos( ) {
	...
	if ( keyboard->getKeyState( "W" ) ) {
		_pos += Vector( _dir.x, 0, _dir.z ) *  MOVE_SPEED;
	}
	if ( keyboard->getKeyState( "S" ) ) {
		_pos += Vector( _dir.x, 0, _dir.z ) * -MOVE_SPEED;
	}
	if ( keyboard->getKeyState( "A" ) ) {
		_pos += _dir.cross( Vector( _dir.x, _dir.y + 100, _dir.z ) ).normalize( ) *  MOVE_SPEED;
	}
...
void Player::updateEye( ) {
	Vector camera_pos = _pos + Vector( 0, _HEIGHT, 0 );

	CameraPtr camera = Camera::getTask( );
	camera->setCamera( camera_pos * MIRI_TO_METER_UNIT, ( camera_pos + _dir ) * MIRI_TO_METER_UNIT );
}
